...
void SolidPropertyManager::buildPanel()
{
    ...
    summaryLabel_ = new QLabel(
        tr("El editor usa propiedades oficiales del documento FCStd y una transacción reversible."),
        panel_);
    ...
    panel_->setStyleSheet(QStringLiteral(R"QSS(
        QWidget#SolidFreeCADPropertyManagerWidget { background: #f5f6f7; color: #202326; }
        QLabel#SolidFreeCADPropertyObject { font-size: 14px; font-weight: 700; color: #18222b; }
        QLabel#SolidFreeCADPropertyType { color: #39759b; font-weight: 600; }
        QLabel#SolidFreeCADPropertySummary { color: #5d666d; }
...
void SolidPropertyManager::enhanceModelTree()
{
    ...
    QDockWidget* modelDock = comboView
        ? qobject_cast<QDockWidget*>(comboView->parentWidget())
        : nullptr;
    if (!modelDock) {
        return;
    }
    ...
    for (QTreeView* tree : comboView->findChildren<QTreeView*>()) {
        ...
        tree->setStyleSheet(QStringLiteral(R"QSS(
            QTreeView { background: #ffffff; alternate-background-color: #f7f8f9; border: 0; outline: 0;
                        selection-background-color: #d7eaf7; selection-color: #101820; }
            QTreeView::item { min-height: 22px; padding: 1px 3px; }
            QTreeView::item:hover { background: #edf5fa; }
            QTreeView::item:selected { border-left: 3px solid #2d84b8; }
        )QSS"));
        if (tree->header()) {
...
{
    if (isFreeCADType(object, "Sketcher::SketchObject")) {
        return QStringLiteral("Sketch");
    }
    if (isFreeCADType(object, "PartDesign::Pad")) {
        return QStringLiteral("Pad");
    }
    if (isFreeCADType(object, "PartDesign::Pocket")) {
        return QStringLiteral("Pocket");
    }
    return object ? QStringLiteral("Unsupported") : QStringLiteral("None");
}
...
void SolidPropertyManager::refreshSelection()
{
    ...
    panel_->setProperty("SolidObjectName", object ? QString::fromUtf8(object->getNameInDocument()) : QString());

    const QSignalBlocker blockLabel(labelEditor_);
    const QSignalBlocker blockLength(lengthEditor_);
    ...
    if (!object) {
        ...
        summaryLabel_->setText(tr("La selección múltiple queda en modo de inspección para evitar cambios ambiguos."));
        labelEditor_->clear();
    ...
    if (kind == QStringLiteral("Sketch")) {
        summaryLabel_->setText(tr("Abra el editor oficial de Sketcher para modificar geometría y restricciones."));
    }
    else if (kind == QStringLiteral("Pad")) {
        summaryLabel_->setText(tr("Extrusión paramétrica; la aplicación crea una única operación deshacible."));
    }
    else if (kind == QStringLiteral("Pocket")) {
        summaryLabel_->setText(tr("Corte paramétrico mediante propiedades FCStd oficiales."));
    }
    else {
...
void SolidPropertyManager::applyChanges()
{
    ...
    try {
        ...
        setStatus(tr("No se pudieron aplicar los cambios: %1").arg(QString::fromUtf8(exception.what())), true);
    }
